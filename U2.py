...
def clientBot(op):
    try:
        ...
        if op.type == 5:
            ...
            sendMention(op.param1, "Ada Apa Add Saya")
        ...
        if op.type == 13:
            ...
            if clientMid in op.param3:
                ...
                sendMention(op.param1, "Kenapa Invite Saya")

        if op.type in [22, 24]:
            print ("[ 22 And 24 ] NOTIFIED INVITE INTO ROOM & NOTIFIED LEAVE ROOM")
            if settings["autoLeave"] == True:
                sendMention(op.param1, "Do not disturb bitch")
...
# Pembatas Script #
                                ...
                                client.sendMessage(to, "Berhasil mengaktifkan auto sambut")
                            elif cmd == "sambut off":
                                settings["Sambutan"] = False
                                client.sendMessage(to, "Berhasil menonaktifkan auto sambut")
                                ...
                                client.sendMessage(to, "Detect mention is actived")
...
# Pembatas Script #
                                ...
                                if receiver in read['readPoint']:
                                    if read["ROM"][receiver].items() == []:
                                        client.sendMessage(receiver,"Belum ada sider")
                                    else:
                                        chiya = []
        ...
        if op.type == 15:
          if settings["Sambutan"] == True:
            if op.param2 not in admin:
                return
            client.sendText(op.param1,"Good Bye " + cl.getContact(op.param2).displayName +  "\nTelah Meninggal . . . ")
